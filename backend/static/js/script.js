(function($) {
  ...
  var validateSurveyForm = function() {
    ...
    function clear() {
      $form.find('#id_name').removeClass('errors');
      $form.find('#id_name_help_inline').remove();
      $form.find('#id_question_1_choice_help_inline').remove();
    }

    clear();

    if ($form.find('#id_name').val() === '') {
      $form.find('#id_name').addClass('errors');
      $form
        .find('#id_name')
        .after(
          '<span id="id_name_help_inline" class="help-inline"><ul class="errorlist"><li>This field is required.</li></ul></span>'
        );

      errors++;
    }

    if ($form.find('[name=question_1_choice]:checked').length === 0) {
      $form
        .find('[name=question_1_choice]')
        .closest('.col-right')
        .siblings('.col-left')
        .append(
          '<span id="id_question_1_choice_help_inline" class="help-inline"><ul class="errorlist">Please select an option.</ul></span>'
        );
      errors++;
    }
  ...
  $(document).ready(function() {
      ...
      if ($('#survey_form .survey_id').val() == 'add') {
        ...
        $.ajax({
          ...
          success: function(data) {
            $.ajax({
              url:
                '/fileupload/?survey_id=' + $('#survey_form .survey_id').val(),
              type: 'PUT',
              data: formData,
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          for (var i = 0; i < data.questions.length; i++) {
            $('.field-question_' + (i + 1))
              .find('.choice_part input[type="radio"]')
              .each(function() {
                ...
                if (data.questions[i].question_type == '1') {
                  $('.field-question_' + (i + 1))
                    .find('.answer_text_part')
                    .show();
                  $('.field-question_' + (i + 1))
                    .find('select.numberofquestions')
                    .val(data.questions[i].options_count);
                  var answer_text_options = data.answer_text.filter(function(
                    value
                  ) {
                    return value.option_question == data.questions[i].id;
                  });
                  var count = 0;
                  $('.field-question_' + (i + 1))
                    .find('.answer_text_part .answer_part')
                    .each(function() {
                      if (count < data.questions[i].options_count) {
                        ...
                        $(this)
                          .find('textarea')
                          .html(answer_text_options[count].option_text);
                        $(this)
                          .find('select')
                          .val(answer_text_options[count].option_goquestion);
                        $(this)
                          .find('input[type="radio"]')
                          .each(function() {
                            if (
                              $(this).val() ==
                              answer_text_options[count].option_complete
                            ) {
                              $(this).attr('checked', 'checked');
                            }
                          });
                  ...
                  $('.field-question_' + (i + 1))
                    .find('.answer_image_part')
                    .show();
                  $('.field-question_' + (i + 1))
                    .find(
                      '.answer_image_part .numberoption_container input[type="radio"]'
                    )
                    .each(function() {
                      if (
                        parseInt($(this).val()) ==
                        data.questions[i].options_count
                      ) {
                        $(this).attr('checked', 'checked');
                      }
                    });
                  var count = 0;
                  var answer_image_options = data.answer_image.filter(function(
                    value
                  ) {
                    return value.option_question == data.questions[i].id;
                  });
                  $('.field-question_' + (i + 1))
                    .find('.answer_image_part .image_row')
                    .each(function() {
                      if (count < data.questions[i].options_count) {
                        $(this).show();
                        if (answer_image_options[count].option_image != null) {
                          $(this)
                            ...
                            .show();
                          $(this)
                            .find('.image_change span')
                            .html(answer_image_options[count].option_image);
                          $(this)
                            .find('.image_change .option_image_name')
                            .val(answer_image_options[count].option_image);
                          $(this)
                            .find('.image_field_container span')
                            .show();
                        }
                        $(this)
                          .find('input[type="text"]')
                          .val(answer_image_options[count].option_tag);
                        $(this)
                          .find('select')
                          .val(answer_image_options[count].option_goquestion);
                        $(this)
                          .find('input[type="radio"]')
                          .each(function() {
                            if (
                              $(this).val() ==
                              answer_image_options[count].option_complete
                            ) {
                              $(this).attr('checked', 'checked');
                            }
                          });
            ...
            $('.field-question_' + (i + 1))
              .find('textarea.question_text')
              .html(data.questions[i].question_text);
          }
        }
      });
    }
    $('#survey_form .form-row').each(function() {
      ...
      if (
        classname.indexOf('question_') > -1 &&
        parseInt(classname.substring(classname.indexOf('question_') + 9)) >
          parseInt($('#id_questions_count').val())
      ) {
        $(this).hide();
      }
    ...
    $('#removeUsersModal .delete_sweepusers').click(function(e) {
      $(".user_list_form tbody input[type='checkbox']:checked").each(
        function() {
          $.ajax({
    ...
    $(".survey_question_part .choice_part input[type='radio']").click(
      function() {
        ...
        $(this)
          ...
          .each(function() {
            if (
              $(this)
                .parent()
                .find(".numberoption_container input[type='radio']:checked")
                .val() == undefined ||
              image_row_count >
                $(this)
                  .parent()
    ...
    $(
      ".survey_question_part .answer_image_part .numberoption_container input[type='radio']"
    ).click(function() {
      var image_row_count = 0;
      var count_val = $(this).val();
      $(this)
    ...
    $('.btn-reportsurvey').click(function(e) {
      ...
      $.ajax({
        ...
        success: function(res) {
          ...
          $.each(res, function(i) {
            var survey_date = new Date(res[i].fields.survey_enter_time)
            ...
            itemsFormatted.push({
              ...
              survey_time: survey_date[1].substring(1),
              question_1:
                res[i].fields.survey_question_1 == ''
                  ? 'N/A'
                  : question_answer[parseInt(res[i].fields.survey_question_1)],
              question_2:
                res[i].fields.survey_question_2 == ''
                  ? 'N/A'
                  : question_answer[parseInt(res[i].fields.survey_question_2)],
              question_3:
                res[i].fields.survey_question_3 == ''
                  ? 'N/A'
                  : question_answer[parseInt(res[i].fields.survey_question_3)],
              question_4:
                res[i].fields.survey_question_4 == ''
                  ? 'N/A'
                  : question_answer[parseInt(res[i].fields.survey_question_4)],
              question_5:
                res[i].fields.survey_question_5 == ''
                  ? 'N/A'
                  : question_answer[parseInt(res[i].fields.survey_question_5)],
              question_6:
                res[i].fields.survey_question_6 == ''
                  ? 'N/A'
                  : question_answer[parseInt(res[i].fields.survey_question_6)],
              question_7:
                res[i].fields.survey_question_7 == ''
                  ? 'N/A'
                  : question_answer[parseInt(res[i].fields.survey_question_7)],
              question_8:
                res[i].fields.survey_question_8 == ''
                  ? 'N/A'
                  : question_answer[parseInt(res[i].fields.survey_question_8)],
              question_9:
                res[i].fields.survey_question_9 == ''
                  ? 'N/A'
                  : question_answer[parseInt(res[i].fields.survey_question_9)],
              question_10:
                res[i].fields.survey_question_10 == ''
                  ? 'N/A'
                  : question_answer[parseInt(res[i].fields.survey_question_10)]
            });
            $.ajax({
    ...
    $(
      ".field-background_image_after_sweepstake_check input[type='radio']"
    ).change(function() {
      if ($(this).val() == 'no') {
    ...
    $('.saveSettings').click(function() {
      ...
      if (settingType == 'tablet') {
        ...
        var index = 0;
        var tabletCount = $('.sweepItem').size();
        $('.sweepItem').each(function() {
          var sweep_ids = $(this)
            .parent()
            .find('.tablet_sweepids')
            .val();
          if ($(this).hasClass('active')) {
            ...
            success: function() {
              if (index == tabletCount - 1) {
                window.location.href =
                  '/admin/sweepstake?id=' + $('#currentSweepID').val();
              }
              index++;
            }
          });
        ...
        if (window.location.href.indexOf('&key') == -1)
          ...
          url =
            window.location.href.substring(
              0,
              window.location.href.indexOf('&key')
            ) +
            '&key=' +
            $(this).val();
      ...
      $.ajax({
        ...
        success: function(res) {
          ...
          for (var i = 0; i < res.users.length; i++) {
            itemsFormatted.push({
              first_name: res.users[i].first_name,
              last_name: res.users[i].last_name,
              phone: res.users[i].phone,
              email: res.users[i].email,
              address: res.users[i].address,
              suite_po_box: res.users[i].suite_po_box,
              city: res.users[i].city,
