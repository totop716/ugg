...
const logoutAPI = (username, password) => {
  ...
  return client.sendAuthenticatedRequest(
    APIConstants.AuthenticationType.TOKEN,
    {
      username,
      password
    }
...
const isAuthedAPI = () => {
  const path = 'api/v1/isauthed/';
  const url = `${Utils.serverUrl}${path}`;

  const client = new APIClient(url, APIConstants.HTTPMethod.GET);
  return client.sendAuthenticatedRequest(APIConstants.AuthenticationType.TOKEN);
};
...
const getTabletAPI = (id, password) => {
  console.log('getTabletAPI', id, password);
...
const updateTabletID = (tabletData, tablet_id, tablet_password, user_id) => {
  ...
  if (tabletData == null) {
    ...
    url =
      ...
      '&tablet_id_code=' +
      Constants.device_id;
    if (user_id != null) url += `&user_id_id=` + user_id;
    ...
    if (tablet_password != null)
      url +=
        '&password=' + tablet_password + '&confirm_password=' + tablet_password;
    if (user_id != null) url += `&user_id_id=` + user_id;
    users = new APIClient(url, APIConstants.HTTPMethod.PUT);
  }
  console.log('URL', url);
...
const updateTabletStatus = (tablet_id, login) => {
  const path = 'tablets/' + tablet_id;
  const url = `${Utils.serverUrl}${path}/?login_status=` + login;
