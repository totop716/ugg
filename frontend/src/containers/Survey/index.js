...
class Survey extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      sweepstakeData: sweepData,
      admin_user: admin_user[0],
      sweep_user,
      tabletData: tablet_data
    });
  ...
  gotoNextQuiz = () => {
    const answer = this.state.survey_questions[this.state.question_no].answers[
      this.state.selected_answer
    ];
    if (answer.option_complete == '2') {
      const currenttime = new Date();
      const { navigate } = this.props.navigation;
      var survey_answers = this.state.survey_answers;
      var selected_answer = this.state.selected_answer;
          ...
          updateCheckTime(
            ...
            updateSurveyCheckTime(
              ...
              navigate('SweepStake', {
                ...
                comeback: false,
                tabletData: this.props.navigation.getParam('tabletData'),
                sweepstakeData: this.props.navigation.getParam(
                  'sweepstakeData'
                ),
                tabletID: this.props.navigation.getParam('tabletID'),
                user: this.props.navigation.getParam('user'),
      ...
      this.setState({
        question_no: answer.option_goquestion - 1,
        selected_answer: -1,
        prev_question_no: question_no,
        survey_answers
      });
    } else {
      const { question_no } = this.state;
      ...
      <StyleProvider style={getTheme(material)}>
        <View style={styles.container}>
          ...
          <ScrollView
            ...
            contentContainerStyle={styles.content_layout}
          >
            <Modal
            ...
            >
              <View style={styles.logoutModalContainer}>
                <Text style={styles.thankyouText}>
                  The survey is optional. If you quit the survey you will still
                  be checked in.
                </Text>
                <View style={styles.buttonsContainer}>
                  <Button style={styles.button} onPress={this.closeCancelBox}>
                  ...
                  >
                    <Text
                      style={[styles.button_text, styles.active_button_text]}
                    >
                      Skip Survey
                    </Text>
                  </Button>
            ...
            <View style={styles.topBar}>
              ...
              <Text style={styles.topBarText}>
                {this.state.question_no + 1} of{' '}
                {this.state.survey_questions != null &&
                  this.state.survey_questions.length}
              </Text>
            </View>
            <View style={styles.survey_content}>
              <Text style={styles.servey_question}>
                {this.state.question_no + 1}.){' '}
                {this.state.survey_questions != null &&
                  this.state.survey_questions[this.state.question_no].question}
              </Text>
              {this.state.survey_questions != null &&
              this.state.survey_questions[this.state.question_no].answers
                .length <= 8 ? (
                <View
                  style={[
                    styles.answers_container,
                    this.state.survey_questions != null &&
                      this.state.survey_questions[this.state.question_no]
                        .type == 2 &&
                      styles.answer_img_container
                  ]}
                >
                  {this.state.survey_questions != null &&
                    this.state.survey_questions[
                      ...
                      this.state.survey_questions[this.state.question_no]
                        ...
                        <TouchableOpacity
                          ...
                          key={index}
                          onPress={() => {
                            this.setState({ selected_answer: index });
                          }}
                        >
                          <View
                            style={[
                              ...
                              this.state.selected_answer == index && {
                                backgroundColor:
                                  '#' +
                                  this.state.sweepstakeData.primary_hex_color,
                                borderColor:
                                  '#' +
                                  this.state.sweepstakeData
                    ...
                    this.state.survey_questions[
                      ...
                      <TouchableOpacity
                        style={[
                          ...
                          index == 0 &&
                            this.state.survey_questions[this.state.question_no]
                              .answers.length == 9 && { width: '100%' },
                          index >=
                            this.state.survey_questions[this.state.question_no]
                              .answers.length -
                              8 && { width: '24%', height: 150 },
                          index < 3 &&
                            this.state.survey_questions[this.state.question_no]
                              .answers.length == 11 && {
                              width: '36%',
            ...
            <View style={styles.button_container}>
              <TouchableOpacity
                style={styles.button}
                onPress={this.showCancelBox}
              >
                <Text style={styles.button_text}>Skip Survey</Text>
              </TouchableOpacity>
